...
# GoFile upload — returns ONLY the download URL, logs go to stderr
gofile_upload() {
    local FILE_PATH="$1"
    ...
    # Only the URL goes to stdout — end='' prevents trailing newline
...
if [ $BUILD_STATUS -eq 0 ]; then
    ...
    ZIP_FILE=$(find "${OUT_DIR}" -maxdepth 1 \( -name "Avium-*.zip" -o -name "*${DEVICE}*.zip" \) \
